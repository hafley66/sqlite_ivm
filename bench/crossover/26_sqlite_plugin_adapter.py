...
def install(db,initial,extension,metrics,sql_path):
    ...
    if metrics:
        raise ValueError('legacy transactional metrics API is not part of sqlite-ivm 0.3')
    if sql_path:
        Path(sql_path).write_text(';\n'.join(r[0] for r in db.execute('SELECT sql FROM sqlite_schema WHERE sql IS NOT NULL ORDER BY rowid'))+';\n')
    return {'algorithm':'sqlite_ivm persistent relational arrangements','extension_sha256':hashlib.sha256(Path(extension).read_bytes()).hexdigest(),
            'extension_path':str(Path(extension).resolve()),'build':'sqlite-ivm 0.3; extension SHA-256 above',
