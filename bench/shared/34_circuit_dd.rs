...
fn native_inventory(keyed: &[BTreeMap<i64, [i64; 3]>; 3], output_rows: usize) -> Value {
    let reason="DD arrangement/trace record and heap-byte totals are not exposed by this adapter; source/output bags do not represent all internal maintained state";
    let mut relations = Vec::new();
    for (index, name) in ["a", "b", "c"].iter().enumerate() {
        relations.push(json!({"name":name,"kind":"native-collection","role":"source","counted_in_totals":true,"row_count":{"value":keyed[index].len(),"unit":"rows","unavailable_reason":null},"bytes":{"allocated":unavailable("bytes",reason),"data":unavailable("bytes",reason),"index":unavailable("bytes",reason)}}));
    }
    ...
    json!({"schema_version":1,"measured_at":"after-output-validation","outside_timed_region":true,"scope":"adapter-observed DD input collections and consolidated output bag","relations":relations,
      "summary":{"table_count":{"value":0,"unit":"tables","unavailable_reason":null},"index_count":{"value":null,"unit":"indexes","unavailable_reason":reason},"native_collection_count":{"value":4,"unit":"collections","unavailable_reason":null},"total_rows":{"value":keyed.iter().map(BTreeMap::len).sum::<usize>()+output_rows,"unit":"rows","unavailable_reason":null,"partial":true},"rows_by_role":{"source":{"value":keyed.iter().map(BTreeMap::len).sum::<usize>(),"unit":"rows","unavailable_reason":null},"result":{"value":output_rows,"unit":"rows","unavailable_reason":null},"support":{"value":null,"unit":"rows","unavailable_reason":reason}},"table_bytes":unavailable("bytes","DD has no SQL tables"),"index_bytes":unavailable("bytes",reason),"total_relation_bytes":unavailable("bytes",reason)},
      "storage":{"database_file_bytes":unavailable("bytes","volatile DD adapter has no database file"),"wal_file_bytes":unavailable("bytes","volatile DD adapter has no WAL file"),"database_allocated_bytes":unavailable("bytes","volatile DD adapter has no database allocation"),"database_size_scope":"no durable database"},"process_memory":{"rss_bytes":unavailable("bytes","measured by the parent runner as process peak RSS, outside this snapshot")},"limitations":[reason]})
}
fn main() {
    let fixture: Value =
    ...
    timely::execute_directly(move |worker| {
        ...
        let mut inputs = worker.dataflow::<u64, _, _>(|scope| {
            ...
            let result = match fixture["circuit"].as_str().unwrap() {
