...
impl Plan {
    pub(crate) fn source_expression(&self, id: usize, expression: &str) -> Option<(usize, String)> {
        if column_references(expression).is_empty() { return None; }
        let node = &self.nodes[id];
        match &node.kind {
            Kind::Input(source) => Some((*source, substitute_columns(expression, |i| {
                quote(&self.sources[*source].columns[i])
            }))),
            Kind::Map { expressions, .. } => self.source_expression(node.inputs[0],
            ...
            Kind::Join { mode: "inner", .. } => {
                ...
                if refs.iter().all(|i| *i < width) {
                    ...
                    self.source_expression(node.inputs[1], &crate::columns::renumber_columns(expression, |i| i-width))
                } else { None }
            }
            _ => None,
        }
    ...
    pub(crate) fn live_rows(&self, db: &Connection, name: &str, id: usize) -> String {
        ...
        match &node.kind {
            Kind::Input(source) => format!("SELECT {},1 AS __n FROM main.{}",
                self.sources[*source].columns.iter().enumerate()
                    .map(|(i,c)|format!("{} AS c{i}",quote(c))).collect::<Vec<_>>().join(","),
                quote(&self.sources[*source].name)),
            Kind::Map { expressions, predicate } => format!("SELECT {},__n FROM ({}){}",
                expressions.iter().enumerate().map(|(i,e)|format!("{e} AS c{i}")).collect::<Vec<_>>().join(","),
                self.live_rows(db,name,node.inputs[0]),
                predicate.as_ref().map(|p|format!(" WHERE {p}")).unwrap_or_default()),
            Kind::Set("all") => node.inputs.iter().map(|input| self.live_rows(db,name,*input)).collect::<Vec<_>>().join(" UNION ALL "),
            Kind::Fixpoint { .. } => format!("SELECT {cols},1 AS __n FROM {}",table(name,id,node.inputs.len())),
            _ => {
                let sources = (0..node.inputs.len()).map(|side| self.live_input(db,name,id,side,false,false)).collect::<Vec<_>>();
                let statement = self.materialize_from_sources(db,name,id,false,Some(&sources));
                let insert = match statement {
                    MaterializeStatements::Set { insert } | MaterializeStatements::Join { insert, .. } => insert,
                    MaterializeStatements::Group(g) if g.window => g.window_insert,
                    MaterializeStatements::Group(g) if g.limit.is_some() => g.limit_insert.replace("__k=?1", "1"),
                ...
                let aliases = (0..width).map(|i|format!("c{i}")).chain(std::iter::once("__n".into())).collect::<Vec<_>>().join(",");
                format!("WITH __ivm_live({aliases}) AS ({query}) SELECT * FROM __ivm_live")
            }
        }
    }

    pub(crate) fn live_input(&self, db: &Connection, name: &str, id: usize, side: usize, restricted: bool, before: bool) -> String {
        let input = self.nodes[id].inputs[side];
        let width = self.nodes[input].fields.len();
        let cols = columns(width);
        let native = self.native_key_values(id,side);
        let key = if native.is_none() { self.key_sql(id,side).expect("membership key") } else { String::new() };
        let identity = identity_sql(width);
        let filter = if restricted && native.is_none() { format!(" WHERE {key} IN (SELECT __v FROM {} WHERE __i IN (SELECT __k FROM temp.__ivm_touched))",keys_table(name)) } else { String::new() };
        let read = |source: String, weight: &str| {
            if restricted {
                if let Some(keys) = &native {
                    return format!("SELECT {},s.{weight} FROM {}",(0..width).map(|i|format!("s.c{i}")).collect::<Vec<_>>().join(","),self.touched_source(name,id,keys,&source));
                }
            }
            format!("SELECT {cols},{weight} FROM {source}{filter}")
        };
        let current = read(format!("({})",self.live_rows(db,name,input)),"__n");
        let rows = if before {
        ...
        if native.is_some() {
            let key = if matches!(self.nodes[id].kind, Kind::Join { .. }) { "0".into() } else { self.key_lookup(name,id,side) };
