...
## step5-member: the fixpoint member key hashed. It loses

`__k INTEGER` hash plus `__c TEXT` and a plain index, replacing
`__k TEXT NOT NULL UNIQUE`. Six runs, medians. Only `fixpoint` touches this
table and only `fixpoint` moved.

| shape | seconds | db bytes | vs step4b time | vs baseline time | vs baseline bytes |
|---|---|---|---|---|---|
| fixpoint | 2.912 | 25 305 088 | 0.83x | 0.78x | 0.96x |
| group | 0.334 | 671 744 | flat | 0.98x | 0.77x |
| distinct | 0.213 | 1 282 048 | flat | 0.88x | 1.03x |
| join | 0.181 | 1 126 400 | flat | 0.92x | 0.84x |

21 percent slower than step 4b for 4 percent off disk. The 26 MB shape the card
called the prize gave back almost nothing.

### Why the member table is not the same problem as `__r`

`__r` had no index job beyond equality, so a hash replaced it cleanly. The
member `__k` was doing dedup work in the b-tree: `INSERT OR IGNORE` against
`UNIQUE(__k)` rejected a duplicate derivation in one seek. A hash cannot carry
a UNIQUE constraint, because a collision is legal, so the dedup had to move
into the statement as `GROUP BY` on the composite plus a `NOT EXISTS`
anti-join. That trades one b-tree seek for a sort of every derived row, every
round of the closure. The sort is the 21 percent.

Space barely moved because the composite never left the table. Before: `__k`
TEXT in the row and the same TEXT in the UNIQUE index. After: `__c` TEXT in the
row, plus 8 bytes of hash in the row and 8 more in the index. Only the index
copy of the text was saved, and the two new integer copies ate most of it.

### The three shapes for a member key

| shape | dedup | index holds | fixpoint seconds |
|---|---|---|---|
| `__k TEXT UNIQUE`, today | b-tree, one seek | the composite | 2.277 |
| `UNIQUE(__k,__c)`, hash first | b-tree, one seek | 8 bytes then the composite | not measured |
| `__k` hash, plain index | sort plus anti-join | 8 bytes | 2.912 |

The middle row is the only shape that keeps b-tree dedup and shortens the seek,
and it does not satisfy this card's wording, which asks for text out of every
index. Reverted; the card's acceptance is wrong for this table the same way it
was wrong for `__r`, and for a different reason.
