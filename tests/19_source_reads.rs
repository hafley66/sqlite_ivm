...
use hafley_observe::{sqlite::{instrument, silence, SQLITE_TARGET}, CountRecorder};
...
fn join_deltas_probe_sources_and_never_write_input_copies() -> Result<()> {
    ...
    assert_eq!(read("SELECT * FROM result ORDER BY 1")?,read(&format!("{query} ORDER BY 1"))?);
    let statements = recorder.event_sums(SQLITE_TARGET,tracing::Level::DEBUG,"drain","view",Some("sql"));
    let mut plans = Vec::new();
    for (_,sql) in statements.keys() {
        for encoding in ["json_array(", "json_object(", "json_each(", "sqlite_ivm_hash("] {
            assert!(!sql.contains(encoding),"serialized join/group/result key: {sql}");
        }
        if sql.starts_with("UPDATE main.\"result_state\"") || sql.starts_with("DELETE FROM main.\"result_state\"") {
            let update_plan = hafley_observe::sqlite::query_plan(&db,sql)?;
            assert!(update_plan.iter().any(|p|p.contains("INTEGER PRIMARY KEY (rowid=?)")),"{update_plan:#?}");
            assert!(!update_plan.iter().any(|p|p.starts_with("SCAN s")),"{update_plan:#?}");
        }
        if sql.starts_with("INSERT INTO temp.__ivm_out_") && sql.contains(" JOIN ") {
            plans.extend(hafley_observe::sqlite::query_plan(&db,sql)?);
        }
        assert!(!sql.starts_with("INSERT INTO main.\"result_op") || sql.contains("__safe"),"{sql}");
        assert!(!sql.starts_with("UPDATE main.\"result_op"),"{sql}");
    }
    for source in ["fact","dimension"] {
        assert!(plans.iter().any(|p|p.contains(source) && p.contains("USING INDEX __ivm_result_source")),"{source}: {plans:#?}");
    }
    Ok(())
}
...
fn composite_keys_use_native_cells_and_group_rowids_survive_updates() -> Result<()> {
    ...
    let rows = |sql: &str| -> Result<Vec<Vec<Value>>> {
        ...
        let values = s.query_map([],|r|(0..width).map(|i|r.get(i)).collect())?.collect();
        values
    };
    let ids = rows("SELECT rowid,k,tag FROM result ORDER BY k,tag")?;
    let indexes: Vec<String> = db.prepare("SELECT definition FROM __ivm_objects WHERE view_name='result' AND object_type='index'")?
        .query_map([],|r|r.get(0))?.collect::<Result<_>>()?;
    ...
    for source in ["fact","dimension"] {
        assert!(indexed_columns.contains(&(source.into(),"k,tag".into())),"{indexed_columns:#?}");
    }
    assert!(indexes.iter().all(|s|!s.contains("json_") && !s.contains("sqlite_ivm_hash")),"{indexes:#?}");
    let native: (i64,i64) = db.query_row("SELECT count(*),count(__v) FROM result_keys",[],|r|Ok((r.get(0)?,r.get(1)?)))?;
    ...
    db.create_scalar_function(c"sqlite_ivm_row_check",-1,
    ...
    assert_eq!(rows("SELECT * FROM result ORDER BY k,tag")?,rows(&format!("{query} ORDER BY f.k,f.tag"))?);
    Ok(())
}
