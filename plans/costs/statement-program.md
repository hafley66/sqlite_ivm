...
18f8971, measured, not landed on main.
...
(all three after below all three before) fails; stopped without tuning.
121425 spans per run on both sides, identical statement count. The removed
text building sits inside the noise of sqlite=debug logging at this test
size. A decisive probe needs logging off (plain wall per
`scripts/statement-costs.sh`) or a sampler on the release binary.
